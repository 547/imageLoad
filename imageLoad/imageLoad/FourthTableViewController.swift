...
class FourthTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cacheSerializer = DefaultCacheSerializer.default
        //MARK:cacheSerializer 和 scaleFactor 组合可以处理下载后缓存的图片的size， PS:scaleFactor的值越大，处理后的图片越小
        cell.imageView?.kf.setImage(with: url, placeholder: #imageLiteral(resourceName: "placeholder"), options: [.scaleFactor(4.0),.cacheSerializer(cacheSerializer),], progressBlock: nil, completionHandler: nil)
        return cell
    }
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        tableView.deselectRow(at: indexPath, animated: true)
        if let cell = tableView.cellForRow(at: indexPath){
            print("====didSelectRowAt======\(cell.imageView?.image?.size ?? CGSize.zero)============")
        }
    }
